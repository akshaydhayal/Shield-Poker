...
export async function GET(request: Request) {
  ...
  if (!API_KEY) {
    return NextResponse.json({ error: 'Navigation failed: Tapestry API key missing' }, { status: 500 });
  }
  ...
  try {
    const response = await fetch(`${API_URL}/profiles/?walletAddress=${walletAddress}&apiKey=${API_KEY}`, {
      headers: {
